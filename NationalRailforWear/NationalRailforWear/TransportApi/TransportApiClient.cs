...
namespace TransportAPISharp
{
    ...
    public class TransportApiClient : IDisposable
    {
        ...
        /// <summary>
        /// Returns places near a given geographic position.
        /// </summary>
        ...
        /// <param name="type">A type or comma separated set of types to limit the search results to. The default is to match on all types.</param>
        ...
        public async Task<PlacesNearResponse> PlacesNear(double lat, double lon, string type)
        {
            var task = await _httpClient.GetAsync(BaseUrl + "/uk/places.json?"
                + $"app_id={_appId}&app_key={_appKey}"
                + $"&lat={lat}&lon={lon}&type={type}");

            var jsonString = await task.Content.ReadAsStringAsync();
            return deserializeResponse<PlacesNearResponse>(jsonString);
        }

        /// <summary>
        /// Returns live train departures from the specified station
        /// </summary>
        /// <param name="lat">The latitude of the position in question.</param>
        /// <param name="lon">The longitude of the position in question.</param>
        /// <param name="type">A type or comma separated set of types to limit the search results to. The default is to match on all types.</param>
        /// <returns>A <c>BusStopsNearResponse</c> class, or null if there was an error.</returns>
        /// <remarks>If there is an error, <c>LastError</c> will contain the error message.</remarks>
        public async Task<LiveTrainsResponse> TrainsLive(string station_code, string calling_at=null)
        {
            //Create Url
            string _requestUrl = BaseUrl + $"/uk/train/station/{station_code}/live.json?"
                + $"app_id={_appId}&app_key={_appKey}";
            _requestUrl = string.IsNullOrEmpty(calling_at) ? _requestUrl : _requestUrl + $"&calling_at={calling_at}";

            var task = await _httpClient.GetAsync(_requestUrl);

            var jsonString = await task.Content.ReadAsStringAsync();
            return deserializeResponse<LiveTrainsResponse>(jsonString);
        }
